...
hook.Add("HUDPaint", "PHE.MainHUD", function()
	state = GetConVar("ph_hud_use_new"):GetBool()
	...
	if IsValid(LocalPlayer()) and LocalPlayer():Alive() and state and LocalPlayer():IsPlaying() then
		-- Begin Player Info
	...
	if IsValid(LocalPlayer()) and IsValid(ava) and (
		not LocalPlayer():Alive() or
		not state or
		(LocalPlayer():Team() == TEAM_SPECTATOR or LocalPlayer():Team() == TEAM_UNASSIGNED)
	) then
