...
function GM:VotePlayMap(ply, map)
	if not map or
	not GAMEMODE.WinningGamemode or
	not GAMEMODE:InGamemodeVote() or
	not GAMEMODE:IsValidGamemode(GAMEMODE.WinningGamemode, map) then
		return
	end
...
function GM:GetFractionOfPlayersThatWantChange()
	...
	for k, pl in pairs(Humans) do
		if pl:GetNWBool("WantsVote") then
			WantsChange = WantsChange + 1
		end

		-- Don't count players that aren't connected yet
		if not pl:IsConnected() then
...
function GM:GetVotesNeededForChange()
	...
	return math.ceil(FractionNeeded * NumHumans) - WantsChange
end
...
function GM:VoteForChange(ply)
	if GetConVar("fretta_voting"):GetInt() == 0 or
	ply:GetNWBool("WantsVote") then
		return
	end
...
timer.Create("VoteForChangeThink", 10, 0, function()
	if GAMEMODE then GAMEMODE.CountVotesForChange(GAMEMODE) end
end)
