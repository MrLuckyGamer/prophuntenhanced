...
function GM:DrawPlayerRing(pPlayer)
	if not IsValid(pPlayer) or
	not pPlayer:GetNWBool("DrawRing", false) or
	not pPlayer:Alive() then
		return
	end
...
function GM:GetTeamColor(ent)
	...
	local entTeam = TEAM_UNASSIGNED
	if ent.Team and IsValid(ent) then entTeam = ent:Team() end
	return GAMEMODE:GetTeamNumColor(entTeam)
