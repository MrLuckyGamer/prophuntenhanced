...
function GM:OnRoundEnd( num )
	...
	if ( fretta_waitforplayers:GetBool() ) then
		...
		-- Set to false
		if ( ( team.NumPlayers( TEAM_HUNTERS ) < 1 ) or ( team.NumPlayers( TEAM_PROPS ) < 1 ) ) then
		...
		-- Set to true
		if ( ( team.NumPlayers( TEAM_HUNTERS ) >= 1 ) and ( team.NumPlayers( TEAM_PROPS ) >= 1 ) ) then
...
function GM:AddRoundTime(iAddedTime)
    ...
    timer.Adjust("RoundEndTimer", GetGlobalFloat("RoundEndTime") - GetGlobalFloat("RoundStartTime"), 0, GAMEMODE.RoundTimerEnd)
...
function GM:PreRoundStart(iNum)
    ...
    timer.Create("RoundStartTimer", GAMEMODE.RoundPreStartTime, 1, GAMEMODE.RoundStart)
...
function GM:RoundStart()
    ...
    timer.Create("RoundEndTimer", roundDuration, 0, GAMEMODE.RoundTimerEnd)
    timer.Create("CheckRoundEnd", 1, 0, GAMEMODE.CheckRoundEnd)
...
function GM:CheckPlayerDeathRoundEnd()
    if not GAMEMODE.RoundBased or
    not GAMEMODE:InRound() then
        return
    end
...
function GM:CheckRoundEndInternal()
    ...
    timer.Create("CheckRoundEnd", 1, 0, GAMEMODE.CheckRoundEndInternal)
end
...
function GM:RoundTimerEnd()
    ...
    if not GAMEMODE.TeamBased then
        ...
        GAMEMODE:RoundEndWithResult(ply and ply or -1, "Time Up")
    else
