...
function GM:ShowHelp()
    if not IsValid(Help) then
        ...
        Help.lblFooterText.Think = function(panel)
            ...
            if GAMEMODE.RoundBased and CurTime() > GAMEMODE:GetTimeLimit() then
                panel:SetText(PHE.LANG.MISC.NOTIMELEFT)
                return
            end
        ...
        if GetConVar("fretta_voting"):GetInt() ~= 0 then
            local btn = Help:AddSelectButton(PHE.LANG.DERMA.RTV, function()
        ...
        if GAMEMODE.TeamBased then
            local btn = Help:AddSelectButton(PHE.LANG.DERMA.CHANGETEAM, function()
                GAMEMODE:ShowTeam()
            end)
        ...
        if not GAMEMODE.TeamBased and GAMEMODE.AllowSpectating then
            if LocalPlayer():Team() == TEAM_SPECTATOR then
                local btn = Help:AddSelectButton("Join Game", function()
                    RunConsoleCommand("changeteam", TEAM_UNASSIGNED)
                end)

                btn.m_colBackground = Color(120, 255, 100)
            else
                local btn = Help:AddSelectButton("Spectate", function()
        ...
        if IsValid(LocalPlayer()) then
            ...
            if Classes and #Classes > 1 then
                local btn = Help:AddSelectButton("Change Class", function()
                    GAMEMODE:ShowClassChooser(LocalPlayer():Team())
                end)
        ...
        if GAMEMODE.SelectColor then
            local function CreateColorPanel()
                ...
                for name, colr in pairs(list.Get("PlayerColours")) do
                    ...
                    icon.PaintOver = function()
                        if GetConVar("cl_playercolor"):GetString() == name then
                            surface.SetDrawColor(Color(255, 210 + math.sin(RealTime() * 10) * 40, 0))
                            surface.DrawOutlinedRect(4, 4, icon:GetWide() - 8, icon:GetTall() - 8)
                            surface.DrawOutlinedRect(3, 3, icon:GetWide() - 6, icon:GetTall() - 6)
                        end
